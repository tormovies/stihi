...
### 2.5. Планировщик (cron)

Чтобы по расписанию выполнялась SEO-оптимизация стихов через DeepSeek (каждые 10 минут) и другие задачи, в crontab должна быть одна строка (Laravel сам запускает запланированные команды):

```bash
* * * * * cd /home/a/adminfeg/stihotvorenie.su/laravel && php8.3 artisan schedule:run >> /dev/null 2>&1
```

Проверка списка задач: `php8.3 artisan schedule:list`

### 2.6. Кеш для sitemap (админка и сайт должны видеть один кеш)
...
### 2.7. Смена пароля админа

На сервере PHP вызывается как **php8.3** (не `php`). Список пользователей и смена пароля:

```bash
cd /home/a/adminfeg/stihotvorenie.su/laravel

# Посмотреть email админа
php8.3 artisan user:list

# Сменить пароль (подставьте нужный email и новый пароль)
php8.3 artisan user:change-password admin@stihotvorenie.su 'новый_пароль'
```

Локально на Windows, если по умолчанию в консоли стоит старая версия PHP, используйте PHP 8.2+ (например путь к `php.exe` нужной версии или алиас, если настроен).

---

## Обновление после git pull (регулярный деплой)

После каждого пуша в репозиторий на сервере выполнить:
